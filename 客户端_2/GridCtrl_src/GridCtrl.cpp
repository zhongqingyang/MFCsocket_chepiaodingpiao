...
BOOL CGridCtrl::SetCustomFixedRow(int nFixedRows)//lgp add
{
    ASSERT(nFixedRows >= 0);
    ResetSelectedRange();
    // Force recalculation
...
BOOL CGridCtrl::SortItems(int nCol, BOOL bAscending, LPARAM data /* = 0 */)
{
	...
	sorttype = bAscending;//fixed by hc@cczu.edu.cn
    ResetSelectedRange();
    SetFocusCell(-1, - 1);
...
BOOL CGridCtrl::SortItems(PFNLVCOMPARE pfnCompare, int nCol, BOOL bAscending, LPARAM data,
                          int low, int high)
{
	if(nCol==0)// fixed by hc@cczu.edu.cn 
		return FALSE;
	
	if(tbtype == 0)//合同信息，13 金额
	{
		if( nCol==0 || nCol==13)
	...
	if(tbtype == 2)//财务回款信息，13 金额
	{
		if( nCol==0 || nCol==10 || nCol==12 || nCol==13)
			SetCompareFunction(CGridCtrl::pfnCellNumericCompare);
		else
			SetCompareFunction(CGridCtrl::pfnCellTextCompare);
	}

	if(tbtype == 3)//支付信息，13 金额
	{
		if( nCol==0 || nCol==4 || nCol==10 || nCol==11 || nCol==12 || nCol==13)
			SetCompareFunction(CGridCtrl::pfnCellNumericCompare);
		else
			SetCompareFunction(CGridCtrl::pfnCellTextCompare);
	}
	...
	{
		int kcount = 0;
        //high = GetRowCount() - 1;//fixed by hc@cczu.edu.cn
		for( int ss=1;ss<GetRowCount();ss++)
		{
			if( GetItemText(ss,nCol)!="")
			{
				kcount = ss;
			}
		}
    ...
    while (lo <= hi)
    {
        ...
        if (lo <= hi)
        {
			...
			/*	for (int col = 1; col < GetColumnCount(); col++)
                {
					CGridCellBase *pCell = GetCell(hi, col);
					if(atoi(pCell->GetText()) >0.000001)
                    needchange = true;
                }*/
...
void CGridCtrl::OnMouseMove(UINT /*nFlags*/, CPoint point)
{
    ...
    {
        if (m_bAllowColumnResize && MouseOverColumnResizeArea(point) && point.x>40)//point.x>3 fix by hc@cczu.edu.cn
        {
...
void CGridCtrl::OnLButtonDown(UINT nFlags, CPoint point)
{
    ...
    if (m_MouseMode == MOUSE_NOTHING)
    {
        if (m_bAllowColumnResize && MouseOverColumnResizeArea(point) && point.x>40)//fix by hc@cczu.edu.cn
        {
            if (m_MouseMode != MOUSE_OVER_COL_DIVIDE)
            {
#ifndef _WIN32_WCE_NO_CURSOR
    ...
    if (m_MouseMode == MOUSE_OVER_COL_DIVIDE && point.x>40) // sizing column  && point.x>2 by fix hc@cczu.edu.cn
    {
...
void CGridCtrl::OnLButtonUp(UINT nFlags, CPoint point)//hc
{
    ...
    else if (m_MouseMode == MOUSE_SIZING_COL)
    {
        ...
        //m_LeftClickDownPoint.x>4 fix by hc@cczu.edu.cn
        if (m_LeftClickDownPoint.x>3 && m_LeftClickDownPoint != point && (point.x != 0 || point.y != 0)) // 0 pt fix by email1@bierling.net
        {   
            CPoint start;
            if (!GetCellOrigin(m_LeftClickDownCell, &start))
                return;
...
void CGridCtrl::OnFixedRowClick(CCellID& cell)
{
	...
	if( cell.col == 0)// fixed by hc@cczu.edu.cn 禁止第一列排序
		return;
    if (GetHeaderSort())
    {
        CWaitCursor waiter;
